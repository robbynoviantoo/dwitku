    ...
    <div className="p-4 md:p-8 max-w-7xl lg:max-w-full mx-auto">
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5 font-normal">
            <span className="font-semibold text-green-600 dark:text-green-400">{income}</span> {t("transactions.income").toLowerCase()} ·{" "}
            <span className="font-semibold text-red-500 dark:text-red-400">{expense}</span> {t("transactions.expense").toLowerCase()}
          </p>
        </div>
        {canEdit && (
          <button
            onClick={() => setDialog({ open: true })}
            className="flex items-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-xl transition-all shadow-sm cursor-pointer"
          >
      ...
      <div className="flex gap-1 bg-zinc-100/80 dark:bg-zinc-800 p-1 rounded-xl w-fit mb-6 border border-zinc-200/60 dark:border-zinc-700">
        {[
          ...
          >
            {tab.label}
            <span className={cn("ml-1.5 text-xs", filter === tab.key ? "text-green-600 dark:text-green-400 font-bold" : "opacity-50")}>{tab.count}</span>
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        >
          {(filtered as Category[]).map((cat) => (
            <div
              key={cat.id}
              className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm p-4 flex items-center gap-3.5 group hover:shadow-md hover:border-zinc-200 dark:hover:border-zinc-700 transition-all"
            >
              {/* Emoji icon */}
              <div
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">{cat.name}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  {cat.type === "INCOME" ? (
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-green-50 dark:bg-green-950 text-green-600 dark:text-green-400">{t("transactions.income")}</span>
                  ) : (
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 dark:bg-red-950 text-red-500 dark:text-red-400">{t("transactions.expense")}</span>
                  )}
                  <span className="text-xs text-zinc-400 dark:text-zinc-400">{cat._count.transactions} {t("transactions.found")}</span>
                </div>
              </div>
              ...
              {canEdit && !cat.isDefault && (
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  <button
                    ...
                    className="p-1.5 text-zinc-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors cursor-pointer"
                  >
              ...
              {cat.isDefault && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-300 shrink-0">
